...
export let options = {
  ...
  stages: [
    { duration: "10s", target: 20 }, // Ramp-up phase
    { duration: "20s", target: 50 }, // Increase to high traffic
    { duration: "30s", target: 100 }, // Stress test peak
    { duration: "20s", target: 50 }, // Recovery phase
    { duration: "10s", target: 10 }, // Cool down
  ],
};

// Base URL
const BASE_URL = "http://localhost:3000/api/regions";
...
// List of API endpoints for stress testing
const endpoints = [
  {
    name: "Search Region",
    path: "/",
  ...
  {
    name: "Districts List",
    path: `/provinces/${provinceId}/cities/${cityId}/districts`,
  },
  {
    name: "Villages List",
    path: `/provinces/${provinceId}/cities/${cityId}/districts/${districtId}/villages`,
  },
];
...
export default function () {
  for (const endpoint of endpoints) {
    ...
    check(res, {
      ...
      [`⚡ ${endpoint.name} - Response time < 800 ms`]: (r) =>
        r.timings.duration < 800,
      [`📊 ${endpoint.name} - Response has data`]: () => hasData,
      ...errorChecks,
    });

    sleep(0.5); // Simulate real user behavior with a short delay
  }
}
